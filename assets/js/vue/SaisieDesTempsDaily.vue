<template>
  <div class="saisie-des-temps saisie-des-temps-daily">
    <h2 class="mb-4 d-flex flex-row align-items-center justify-content-between">
      ...
      <span
        v-tippy="{
          content: isCurrentDate
            ? 'Impossible de saisir les temps passés dans le futur.'
            : null,
        }"
      >
        <button
    ...
    <p
      v-if="cra && cra.isUserBelongingToSociete === false"
      class="lead text-center"
    >
      Impossible de saisir les temps de cette période car vous n'étiez pas dans
      la société.<br />
      Dans le cas contraire, contactez votre administrateur RDI-Manager ou votre
      N+1 pour mettre à jour votre date d’entrée dans la société.
    </p>

    <p
      v-if="
        cra &&
        cra.tempsPasses.length === 0 &&
        cra.isUserBelongingToSociete === true
      "
      class="lead text-center"
    >
      Cette période, vous n'avez pas de temps à saisir car vous n'étiez
      contributeur sur aucun projet.
    </p>
    ...
    >
      <div class="message-validation text-center">
        <p v-if="cra.tempsPassesModifiedAt" class="text-success">
          ...
          {{
            t("month_validated_on", {
              month: formatDate(cra.tempsPassesModifiedAt),
            })
          }}
        </p>
      </div>
        ...
        <tbody>
          <tr v-for="tempsPasse in cra.tempsPasses" :key="tempsPasse.id">
            <td>
              {{
                t("project_heading", {
                  project_name: tempsPasse.projet.acronyme,
                })
              }}
            </td>
            <td
              v-for="(_, i) in tempsPasse.pourcentages"
              :key="tempsPasse.id + '-' + i"
            >
              <p>
                <input
      ...
      <p v-if="!validCra(cra)" class="text-danger text-center">
...
export default {
  ...
  mounted() {
    ...
    if (4 === path.length) {
      ...
      this.selectedDate = strategy.calculateInitialDate(
        new Date(parseInt(year, 10), parseInt(month, 10) - 1, 7)
      );
    }
  },
  ...
  methods: {
    ...
    submitCra(e) {
      ...
      this.cra.tempsPasses = this.cra.tempsPasses.map((tempsPasse) => {
        tempsPasse.pourcentages = tempsPasse.heures.map(
          (h) => (h * 100.0) / this.hoursPerDays
        );

        return tempsPasse;
      });

      strategy.apiSaveCra(this.cra, this.selectedDate).then(() => {
        ...
        if (this.urlToAbsences) {
          const url = this.urlToAbsences
            ...
            .replace(
              "99",
              ("0" + (this.selectedDate.getMonth() + 1)).substr(-2)
            );
          addHtmlFlashMessage(
            "warning",
            t("enter_absences_if_taken_this_month", {
